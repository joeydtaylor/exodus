...
type ReceiverPipeline struct {
	...
	Output       string   `toml:"output"`       // named wire produced by this pipeline
}
...
// NOTE: typeReg is expected to be defined elsewhere in this package as your datatype registry.
// We only read from it here.
func (c *Config) Validate() error {
	// Routes are optional; normalize/validate if present.
	...
	// Receiver validation + normalization; collect wires.
	...
	for i := range c.Receivers {
		...
		// OAuth mode-specific requirements
		if rc.OAuth != nil {
			o := rc.OAuth
			switch o.Mode {
			case "", "off":
		...
		for j := range rc.Pipeline {
			...
			if out == "" {
				return fmt.Errorf("receiver %d pipeline %d: output wire name required", i, j)
			}
	...
	// Validate sinks and cross-check inputs with wires
	for i := range c.Sinks {
		s := c.Sinks[i]
		if len(s.Inputs) == 0 {
			return fmt.Errorf("sink %d: at least one input wire required", i)
		}
		// every sink input must exist
		...
		case SinkTypeS3:
			...
			// batch defaults (sane dev defaults)
			if s.S3.Batch == nil {
				s.S3.Batch = &S3Batch{}
			}
			...
			// AWS defaults for LocalStack/dev
			if s.S3.AWS != nil {
				aws := s.S3.AWS
				if aws.EndpointURL != "" && !aws.UsePathStyle {
					aws.UsePathStyle = true // LocalStack & many custom endpoints need this
				}
				if aws.SessionName == "" && aws.RoleARN != "" {
					aws.SessionName = "exodus-session"
				}
