...
func registerHooks(lc fx.Lifecycle, d serverDeps) {
	addr := envOr("SERVER_LISTEN_ADDRESS", ":5000")
...
func main() {
	_ = godotenv.Load()
	if os.Getenv("EXODUS_MANIFEST") == "" {
		os.Setenv("EXODUS_MANIFEST", "manifest.toml")
	}
	...
	fx.New(
		...
		// - typed publisher (adapts to exodus.TypedPublisher)
		fx.Provide(
			fx.Annotate(
		...
		fx.Provide(provideRelayForExodus),

		// Register user datatypes + transformers BEFORE router/receivers boot
		fx.Invoke(apptypes.RegisterAll),
		fx.Invoke(apphandlers.Register),
